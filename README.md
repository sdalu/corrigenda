...
rather than 403 — a route that is switched off should not advertise
that it exists.
...
Nothing here needs a plugin or an integration: the interface
describes itself, so an agent with a shell has enough. **A running
service prints the prompt for you** — the landing page carries one
written for that deployment, with the socket it is actually
listening on, a site it actually accepts reports about, and a last
paragraph that says what the agent is allowed to do there. Copy it
